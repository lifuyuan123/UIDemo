package com.example.administrator.uidemo.view;
 ...
 public class MagicCircle extends View {
    ...
    private int width;
     ...
     private float mInterpolatedTime;//插补时间
     private float stretchDistance;  //拉伸距离
//     private float moveDistance;//移动距离
     private float cDistance;
     private float radius;//半径
     private float c;//控制点偏移量
     private float blackMagic = 0.551915024494f;//贝塞尔值
     ...
     @Override protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
         ...
         centerY = height / 2;
         radius = 100;
         c = radius*blackMagic;//得到贝塞尔偏移量
         stretchDistance = radius;//拉伸距离等于半径
//         moveDistance = radius*(3/5f);
     ...
     @Override protected void onDraw(Canvas canvas) {
         super.onDraw(canvas);
         mPath.reset();
         canvas.translate(radius+20, centerY-120);
